...
## To restrict to giant component, uncomment this line
mdf = mdf %>% filter(component == 1)
...
country_df %>% select(country, docs_sum, docs_mean) %>% arrange(desc(docs_sum)) %>% kable
## Plot
ggplot(data = country_df, aes(x = reorder(country, n_authors), y = n_authors, fill = country)) + 
	geom_bar(stat = 'identity') +
	xlab('country') + ylab('no. authors') +
...
country_df %>% select(country, btwn_sum) %>% arrange(desc(btwn_sum)) %>% kable
