...
plot(country_net, layout = layout_country, 
	 vertex.size = 5 + .05*V(country_net)$n_loops, 
	 ...
	 vertex.color = communities$membership,
	 vertex.frame.color = NA,
	 #vertex.shape = V(country_net)$name %in% western_balkans,
	 edge.curved = TRUE,
	 edge.color = 'grey60',
...
We can also use cluster analysis, based on which areas tend to have the same individual researchers.  

```{r areas_cluster, eval=TRUE}
## Cluster analysis
...
areas_network = mdf %>% select(one_of('id', areas_45)) %>% 
	melt(id.vars = 'id', variable.name = 'area') %>%
	filter(value) %>% 
...
ggplot(data = {missed_df %>% group_by(country.1, country.2) %>% 
	...
	stat_function(fun = function(x) (41 - x)) +
	xlab('') + ylab('') +
